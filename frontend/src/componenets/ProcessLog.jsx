  ...
  const fetchLogs = async () => {
    try {
      ...
      const response = await fetch(
        `/api/process_logs?${params.toString()}`,
        {
          headers: {
      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className="relative w-full max-w-4xl h-3/5 max-h-[600px] bg-white rounded-2xl shadow-2xl overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gray-50">
              <h2 className="text-lg font-semibold text-gray-800">
                {getEntityTitle()} #{entityId} - 操作記錄
              </h2>
              <button
                onClick={onClose}
            ...
            <div className="flex flex-col h-full">
              {isLoading ? (
                <div className="flex-1 flex items-center justify-center">
                  <LoadingSpinner variant="circular" size={30} message="載入記錄中..." />
                </div>
              ) : error ? (
                <div className="flex-1 flex items-center justify-center">
                  <FetchDataFail
                                  ...
                                  <div className="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                                    <div>
                                      <p className="text-sm text-gray-900">
                                        <span className="font-medium">{log.user?.name || "系統"}</span>
                                        <span className="mx-1">{getActionText(log.action)}了</span>
                                        <span className="font-medium">{getEntityTitle()}</span>
                                        {log.field_name && (
